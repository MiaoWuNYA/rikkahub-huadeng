...
> 本项目是 [RikkaHub](https://github.com/rikkahub/rikkahub) 的深度定制分支，已合入上游 **v2.5.2**。
> 上游全部功能原样保留，在此基础上重点强化了 **⚡提示词前缀缓存**、**🧠记忆与长对话**、**🍺酒馆（SillyTavern）兼容**、**🧩插件系统**、**🗼中转站兼容**、**📱手机增强**、**🛡隐私与稳定性** 七大方向。
...
- **插件系统**：QuickJS 沙箱插件，ZIP 一键导入，AI 可直接调用插件工具；支持会话式 HTTP 与图片解码，可做登录态与图像处理
...
| 你想做的事情 | 对应的功能 |
|---|---|
| 在安卓手机上跑酒馆角色卡 / 世界书 | 角色卡（V2/V3/PNG）、世界书 30+ 字段、预设、正则脚本、快速回复全部按 SillyTavern 官方语义导入 |
| 找一个手机上能用的 SillyTavern 客户端 | 本应用就是；酒馆兼容层是核心方向，不是外挂 |
| AI 聊天长对话太贵 / 老是失忆 | 提示词前缀缓存 + 语义记忆 RAG + 三层记忆 + 上下文滚动压缩 |
| 想接入自己的中转站 / 第三方 API | OpenAI / Anthropic / Google / DeepSeek 兼容协议，中转站兼容开关修复常见病态 |
| 想给 AI 加自己的工具 | QuickJS 插件系统，写个 `main.js` 打包 ZIP 就能导入，AI 可直接调用 |
| 想要微信 / QQ 里也能聊 | 微信 Bot（扫码）、QQ Bot（官方 API），复用任意助手的 AI 与记忆 |
| 想要手机上能读出来、能对话的语音 | 豆包 TTS 2.0 + 火山 ASR，支持语音 / 视频通话 |
...
| 开关 | 说明 |
|---|---|
| **中转站兼容** | Gemini 经 OpenAI 兼容中转站（newapi 等）接入时的三类经典病态自动修复，详见下节 |
| **防空回复** | Gemini 空回复自动微扰重试 + 系统提示词入对话流，详见下节 |
| **上下文瞬态内容裁剪** | 超过两轮的网页搜索结果、图片、音视频不再随每次请求发送（占位说明附消息 ID，AI 可用 `read_history_message` 按需取回），大幅减少图片与搜索类长对话的 token 消耗 |
| **清爽简洁模式** | 隐藏情侣空间、生活空间等娱乐功能入口，且不再向 AI 注册对应工具，界面更简洁、上下文更省 token |
| **上下文滚动压缩** | 对话过长时自动把早期消息压缩为摘要以节省 token。关闭后不再自动压缩（助手级开关仍可单独启用） |
| **工具结果截断** | 工具输出超过 32KB 自动截断并保存到文件。中转站导致工具调用异常时可尝试关闭 |
| **系统提示词转义** | 把系统消息中的 `<` `>` 转为 HTML 实体，绕过中转站 WAF 拦截（遇到 `upstream_content_rejected` 时可开启）。一般不需要开 |

### 接入与自动化

微信 Bot、QQ Bot、AI 主动发消息、安全设置、作者注释五个入口。

### 1. 中转站兼容（Proxy Fix）

Gemini 经 OpenAI 兼容中转站接入时的三类经典病态，自动修复：
...
QuickJS 沙箱插件：插件 = ZIP 包（`manifest.json` + `main.js`），设置 → **插件管理**一键导入（两段式预览确认 + SHA-256 完整性校验 + Zip Slip 防护）。

> 插件系统源自 orangechat / Tumin，本分支在其之上补齐了工具型插件所需的沙箱能力。

### 插件能做什么

- manifest 声明的 tools 自动转成 AI 可调用的工具（统一 `plugin_` 前缀，执行需审批）
- **系统提示词注入**：manifest 可声明 `systemPrompt`（直接注入）或 `sections`（每段配一个开关，用户按需启用）
- **详情页数据卡片**：声明 `detailCard` 后，插件详情页会调用该导出函数并渲染成一块实时状态卡片（如卡路里的"今日摄入"）
...
| API | 用途 |
|---|---|
| `fetch` | 一次性网络请求，域名白名单（fail-closed） |
| **`http.*`** | **会话式请求，自带 Cookie 罐**——自动保存每一跳响应的 `Set-Cookie`，并可取二进制（`bytes()` / `base64()`）。用于"先拿 session 再带着它登录"的流程，如教务、论坛 |
| **`image.decode`** | **把图片解成 RGBA 像素数组**（布局同 `canvas.getImageData`），沙箱没有 canvas，逐像素处理（验证码识别、取色）靠它 |
| `dataStore` | 每插件独立的键值存储（`set/get/del/list`） |
| `memoryBank` | 语义记忆读写 |
| `musicPlayer` | 音乐播放控制 |
| `btoa` / `atob` / `TextEncoder` / `TextDecoder` | 编解码 |

沙箱为单线程、工具调用 30 秒超时、详情页卡片 8 秒超时。

### 内置插件

| 插件 | 说明 |
|---|---|
| **卡路里与蛋白质记录** | 跟 AI 说一句"中午吃了碗牛肉面"，热量与蛋白质就都记下来了。每日目标在插件设置里配置，详情页显示今日摄入与各自剩余额度 |
| **云南财经教务系统** | 课表 / 成绩 / 考试安排 / 教务公告 / 空闲教室查询。账号密码在插件设置里填，验证码本地 OCR 自动识别，连续失败则返回图片让用户手输 |

插件包在 [`docs/plugins/`](docs/plugins/)（`calorie.zip`、`ynufe.zip`），开发指南见 [docs/PLUGINS_GUIDE.md](docs/PLUGINS_GUIDE.md)，插件源码同样在该目录下可直接参考。
...
- **注入内容按用户轮冻结**：世界书、技能、跨窗口记忆、OCR 结果、正则 depth 语义等按"用户轮"取值，同一轮内多次请求前缀保持一致
- **历史消息宏渲染按消息冻结**：已进入历史的助手消息，其宏渲染结果不再随当前时间/变量变化
- **滚动压缩摘要追加式**：长对话触发上下文压缩后，摘要以追加方式衔接，token 前缀跨压缩保持稳定
- **注入位置治理**：记忆全量注入移至上下文尾部、Recent Chats 移出前缀区，热路径上不再有易变内容
- **API Key 按会话粘性**：多 Key 时同一会话固定用同一个 Key，避免上游侧缓存因账号切换而失效
...
在上游基础上新增：文件操作、Shell、任务工具、计算器、数据库查询、Python 引擎（Chaquopy）、网页抓取、系统提示词装配器（工具选用指南 / 工作守则）。

云南财经教务系统工具已**移出应用**，改为插件形式提供，见「插件系统 → 内置插件」。

### 工具调用兼容

- **DSML 文本工具调用兼容**：模型不走 function calling、而是把调用写在正文里时，自动解析并执行
- **工具别名映射**：`web_search` → `search_web` 等 DSML 惯用名自动映射到实际注册名
- **工具中文雅称**：工具调用在界面上一律显示中文名（如 `web_search` → 联网搜索）
...
- **已合入上游版本**：rikkahub/rikkahub master（**v2.5.2**）
- **相对 mingli2 分支**：除酒馆增强外，新增提示词前缀缓存、语义记忆 RAG 与上下文滚动压缩、中转站兼容与防空回复、豆包语音、隐私加固等
- **v2.5.4 以来新增**：教务系统移出应用改为插件、卡路里与蛋白质记录插件、插件沙箱的会话式 HTTP 与图片解码能力、详情页数据卡片、更新检查改用 GitHub Releases API
